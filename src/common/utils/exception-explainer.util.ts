...
export class MongooseExceptionExplainer {
  ...
  static explain(error: Error | mongo.MongoServerError): MongooseErrorDetail {
    ...
    if (error.name === 'CastError' && error instanceof Error.CastError) {
      return {
        kind: 'CAST_ERROR',
        message: `The value "${error.value}" is not a valid ${error.kind} for field "${error.path}".`,
        path: error.path,
        value: error.value,
      };
    }
    ...
    if (error.name === 'ValidationError' && error instanceof Error.ValidationError) {
      const messages = Object.values(error.errors).map((err: Error.ValidatorError | Error.CastError) => err.message);
      return {
    ...
    if (error instanceof mongo.MongoServerError && error.code === 11000) {
      const duplicateError = error as mongo.MongoServerError & { keyValue: Record<string, unknown> };
      const field = Object.keys(duplicateError.keyValue)[0];
      const value = duplicateError.keyValue[field];
      return {
        kind: 'DUPLICATE_KEY_ERROR',
        message: `The value "${value}" already exists for field "${field}". Please use a unique value.`,
        path: field,
        value: value,
      };
    }
    ...
    return {
      ...
      message: (error as Error).message || 'An unexpected database error occurred.',
    };
  }
}
